...
class Thresholds {
  ...
  protected function exceeds($value) {
    if (($threshold = $this->sensorInfo->getThresholdValue('critical')) !== NULL && $value > $threshold) {
      $this->message = format_string('exceeds @expected', array('@expected' => $threshold));
      return SensorResultInterface::STATUS_CRITICAL;
    }
    if (($threshold = $this->sensorInfo->getThresholdValue('warning')) !== NULL && $value > $threshold) {
      $this->message = format_string('exceeds @expected', array('@expected' => $threshold));
  ...
  protected function inner_interval($value) {
    if (($low = $this->sensorInfo->getThresholdValue('critical_low')) !== NULL && ($high = $this->sensorInfo->getThresholdValue('critical_high')) !== NULL) {
      if ($value > $low && $value < $high) {
        $this->message = format_string('violating the interval @low - @high', array('@low' => $low, '@high' => $high));
        return SensorResultInterface::STATUS_CRITICAL;
      }
    }
    if (($low = $this->sensorInfo->getThresholdValue('warning_low')) !== NULL && ($high = $this->sensorInfo->getThresholdValue('warning_high')) !== NULL) {
      if ($value > $low && $value < $high) {
        $this->message = format_string('violating the interval @low - @high', array('@low' => $low, '@high' => $high));
        return SensorResultInterface::STATUS_WARNING;
      }
